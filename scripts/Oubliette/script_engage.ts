...
const map = new remapper.Difficulty("EasyStandard.dat", "EasyLawless.dat");
const mapHJD = 1.25
...
const streamVariation_effs: ty.NoteEffect = ef.combineEffects([streamVariation_pop])
...
const zoom_init: ty.Effect<remapper.Wall> = ef.initializePosition()
...
const zoom_wall_efs: ty.Effect<remapper.Wall> = ef.combineEffects([zoom_init,zoom_fast,zoom_hjd_increase,zoom_easydodge])
...
const increasing_walls_cr: ty.Creator<remapper.Wall> = cr.parameterizeCreationByCustomData(increasing_walls_cf,function(dir: number) { return eff.increasingWall(dir)})
...
const shotgun_ef: ty.Effect<remapper.Note> = eff.queueObject()
fn.mapEffect(shotgun_ef)(shotgun_notes)

const intermachinegunHJDinc = 0.25
...
const machinegun_ef: ty.Effect<remapper.Note> = eff.zoomIn()
...
const open_up_ef = eff.openOrClose(200,202,0,0.75)
...
const close_up_ef = eff.openOrClose(202,204,0.75,0)
